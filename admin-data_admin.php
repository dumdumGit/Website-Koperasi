...
$h=mysql_query("select max(id) as id from user");
	...
	?>
	<form method="post" action="form/input_admin.php" enctype="multipart/form-data">

<table>
<tr>
<td><span>ID_Admin</span></td><td>:</td><td><input type="text" id="textfield" name="id_admin" value="<?php echo"$idbaru"; ?>" size="25"></td>
</tr>
...
WHERE level = 'admin' order by id desc");

while($array=mysql_fetch_array($selek)){
echo"

<tr><td>$array[username]</td><td>$array[level]</td><td>$array[id]</td><td><a href='update_data_user_admin.php?id=$array[id]'><input type='image' src='gambar/edit.jpg' width='40px' height='30px'></a><img src='gambar/slash.png' height='20' width='20'><a href='form/delete_admin.php?id=$array[id]'><input type='image' src='gambar/delete.jpg' width='40px' height='30px'></a></tr>
";
}
?>
</div>
